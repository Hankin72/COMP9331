from packet import *
import random
#import asyncio#never to be used
import time
...
def send_packet(socket, header, data=None, pld=None, logger=None):
    ...
    #2_duplicate
    #if      #duplicate
    #If not drop,then send twice,use socket.send(packet.msg) double time
    '''
    if pld is not None:
        duplicate = pld.roll()
        if duplicate < pld.pDuplicate:
            print('duplicate coming!')
    ...
    if pld is not None:
        corrupt = pld.roll()
        if corrupt < pld.pCorrupt:
            corrupted = True
            
            return
    #4_re-ordering
    # if    #judge if % perscent < seed.perscent
    # then cope with reorder things
    
    if pld is not None:
        reorder = pld.roll()
        category = None
        if reorder < pld.pOrder:
            #ordered = pld.maxOrder
            category = 1
            return category
    '''
...
class Logger:
    ...
    def receiver_conclude(self):
        self.file.write("=====================================================\n")
        self.file.write("Amount of (original) Data Transferred (in bytes): {}\n".format(self.data_size))
        self.file.write("Total Segments Received: {}\n".format(self.num_segments + 4))
        self.file.write("Number of (original) Data Segments Received: {}\n".format(self.num_segments))
        self.file.write("Data segments with Bit errors: {}\n".format(self.corrupt))
        self.file.write("Number of duplicate segments received: {}\n".format(self.dup_count))
        self.file.write("Number of Duplicate Acknowledgements received: {}\n".format(self.dup_count))

class Thread(threading.Thread):#!(threading.pip)
    def __init__(self, name, sender):
        threading.Thread.__init__(self)#what's this style?
        self.name = name
        self.sender = sender
